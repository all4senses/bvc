<?php 
  ...
  $extra_data = $teaser_data = bvc_misc_updateArticleExtraData($node, TRUE, 1397495323);
  ...
  if($view_mode == 'side_block_teaser_onPrefaceBottomLatest') {
    ...
    $teaser_data = bvc_misc_getArticleTeaserData('all', $body, $node->nid, 270);

    $author_name = bvc_misc_getNodeAuthor($node);
  ...
  elseif ($view_mode == 'side_block_teaser_latestBlogsOnNews') {
    ...
    //echo '<div class="block-thumb">' . theme('bvc_misc_image_style', array('style_name' => 'block_thumb', 'src' => $main_image['src'], 'path' => $main_image['uri'], 'alt' =>  (@$main_image['alt'] ? $main_image['alt'] : $title), 'title' => $title )) . '</div>' . l($node->title, 'node/' . $node->nid);
                ...
                */
                ...
                echo $submitted;
                  ...
                  elseif ($paths_with_latest_article) {
                    ...
                    echo ($_SERVER['REQUEST_URI'] != '/' ? $type_cations[$node->type] . ' - ' : '') . 'By <span class="author">' , $author_name, '</span>' /*l($author_name, 'user/' . $node->uid, array('attributes' => array('title' => $author_name . '\'s profile', 'class' => 'username')))*/, ' / ', date('F d, Y', $node->created) /*bvc_misc_elapsed_time($node->created)*/;
                  }
                  else {
      ...
      <div class="content <?php echo ($page ? 'page' : 'teaser'); ?>"<?php print $content_attributes; ?>>
        <?php
          ...
          if (isset($content['field_topics']) && (!isset($content['metatags']['keywords']['#attached']['drupal_add_html_head'][0][0]['#value']) || !$content['metatags']['keywords']['#attached']['drupal_add_html_head'][0][0]['#value']) ) {
            ...
            bvc_misc_pushTagsToMetatags('keywords', $content['field_topics']);
          }
          else {
            bvc_misc_addMetatag('news_keywords', $content['metatags']['keywords']['#attached']['drupal_add_html_head'][0][0]['#value']);
          }
          */
          ...
          if (isset($content['metatags']['keywords']['#attached']['drupal_add_html_head'][0][0]['#value']) && $content['metatags']['keywords']['#attached']['drupal_add_html_head'][0][0]['#value']) {
            bvc_misc_addMetatag($keyword_metatag_name, $content['metatags']['keywords']['#attached']['drupal_add_html_head'][0][0]['#value']);
          }
//          elseif (@$content['field_topics']) {
//            bvc_misc_pushTagsToMetatags($keyword_metatag_name, $content['field_topics']);
//          }
                  ...
                  <footer>
                    ...
                    <?php /*  
                      ...
                      <div class="main">
                              <?php 
//                                if ($user->uid != 1) {
//                                  echo bvc_blocks_getSocialiteButtons($url, $title); 
//                                }
                              ?> 
  ...
  <?php /*if ($page && $node->type != 'news_post'): ?>
      ...
      <div class="providers">
        <?php 
          ...
          echo bvc_blocks_getBlockThemed($block_data);
