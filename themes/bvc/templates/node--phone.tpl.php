...
<?php else: ?>
    ...
    <?php 
        ...
        echo '<div class="float share">' . bvc_blocks_getSocialiteButtons($url, $share_title) . '</div>';
        */
    ?>
<?php endif; ?>
      ...
      <?php endif; ?>
          ...
          <?php 
            ...
            $url = 'http://businessvoipcenter.com'. url('node/' . $node->nid);
            ...
            if ($page || $view_mode == 'teaser'  || $view_mode == 'side_block_teaser') {
                ...
                if ($view_mode != 'side_block_teaser') {
                  $authorExtendedData = bvc_misc_loadUserExtendedData($node->uid);
        ...
        <div class="content<?php echo ($view_mode == 'teaser' ? ' teaser' : ''), '" ', $content_attributes; ?>>
           ...
           <?php if ($page): 
              ...
              echo bvc_blocks_getSidebarShareStaticBlock($node, '<span>Share:</span>', 'top bottom');
            ...
            <div id="phone-top">
              ...
              <div class="share">
                      ...
                      <div class="main">
                          <?php //echo bvc_blocks_getSocialiteButtons($url, $title); ?> 
              ...
              */ ?>
              ...
              <div class="data tabs">
                    ...
                    <div id="tabs-1">
                      ...
                      <div class="bvc_votes editor phone">
                        <?php echo 
                                //'<div class="caption"><span><span property="v:reviewer">Editor</span>\'s Overall Rating:</span> <span property="v:rating">' , $editor_overall_rating, '</span>' /* render($content['bvc_rating_overall'])*/ , '<div class="bottom-clear"></div></div>', 
                                render($node->editor->content['bvc_ratings']); 
                        ?>
                        <div class="rate-other">
                          <?php echo '<div class="voters editor"><div class="count" property="v:count">' . (!empty($node->bvc_voters) ? $node->bvc_voters : 1) . '</div></div>';?>
                        </div>
                        ...
                        <?php 
                            ...
                            </span><div class="stars-rating">' . theme('bvc_misc_fivestar_static', array('rating' => $editor_overall_rating * 20, 'stars' => 5, 'tag' => 'overall', 'widget' => array('name' => 'stars', 'css' => 'stars.css'))) . '</div>'; 
                    ...
                    <?php endif; ?>
                    <div id="user-reviews">
                      <?php if (isset($content['bvc_ratings']) && $content['bvc_ratings']): ?>

                            <div class="bvc_votes users_overall"><?php echo '<div class="caption">' . t('Overall Consumer Ratings') . '</div>' . render($content['bvc_ratings']); ?></div>
                            <div class="overall"> 
                              <div class="text">
                                <?php 
                                  //echo '<div class="voters"><div class="title">' . t('Number of Reviews') . ':</div><div class="count" property="v:count">' . $node->bvc_voters . '</div></div>';
                                  echo '<div class="voters"><div class="title">' . t('Number of Reviews') . ':</div><div class="count">' . $node->bvc_voters . '</div></div>';
                                ?>
                                <?php echo '<div class="recommend"><div class="title">' . t('Would recommend') . ': </div><div class="data">' . $node->bvc_recommend . '% ' . t('of all voters') . '</div></div>'; ?>
                                <div class="overall title"><?php $node->field_p_name['und'][0]['value'] . ' ' . t('Overall Rated:'); ?></div>
                              </div>
                              <div class="star-big">
                                <?php 
                                  //echo '<div class="count" content="' . $node->bvc_rating_overall . '" property="v:rating">' . $node->bvc_rating_overall . '</div>' . '<div class="descr">' . t('Out of 5 stars') . '</div>';
                                  echo '<div class="count" content="' . $node->bvc_rating_overall . '" >' . $node->bvc_rating_overall . '</div>' . '<div class="descr">' . t('Out of 5 stars') . '</div>';
                                ?>
                              </div>
                            </div>

                      <?php endif; // end of if (isset($content['bvc_ratings']) && $content['bvc_ratings']): ?>
          ...
          <?php elseif ($view_mode == 'teaser'):  ?> <!-- Else of if ($page): -->
              ...
              <?php
                ...
                bvc_misc_regenerateStyledAndBeautifyImageHtml($node->field_p_image['und'][0]['uri'], 'article_thumbnail_h', $main_image_html, $main_image_html_beautify, $node->title);
          ...
          <?php elseif ($view_mode == 'teaser_phonePicAndRating'): ?> <!-- Second Elseif of if ($page): -->
              ...
              <?php
                  $stars = theme('bvc_misc_fivestar_static', array('rating' => $node->extra_data['editor_rating_overall'], 'stars' => 5, 'tag' => 'overall', 'widget' => array('name' => 'stars', 'css' => 'stars.css')));
      ...
      <?php  if ($page): ?>
        ...
        <footer>
          <div class="share">
              <?php 
                //echo bvc_blocks_getSidebarShareStaticBlock($node, '<span>Share:</span>');
                //echo bvc_blocks_getSidebarShareStaticBlock($node, '<span>Share This Post:</span>', 'bottom');
              echo bvc_blocks_getSidebarShareStaticBlock($node, '', 'bottom');
              ?> 
          </div>
  ...
  //      if ($user->uid) 
        {  
          echo bvc_blocks_getAboutTheAuthor($node->uid); 
        }
