  <div class="main-content page404"> 
      ...
      <div class="content page preface">
        <?php
          ...
          drupal_set_title('Page Not Found. | businessvoipcenter.com');
        
          // Fix ttp:// urls, i don't understand where they are coming from, but seems that via msie6.0
          if (strpos($_GET['destination'], 'ttp://') === 0) {
            $fixed_path = str_replace('ttp://businessvoipcenter.com/', '', $_GET['destination']);
            unset($_GET['destination']);
            watchdog('BVC Redirect ttp:// to http://... 404', $fixed_path . ' ------- >'. print_r($_GET, TRUE) . ' '. print_r($_SERVER, TRUE), NULL, WATCHDOG_WARNING);
            drupal_goto($fixed_path);
          }
          elseif (strpos($_GET['destination'], '___.html') !== FALSE) {
            $fixed_path = str_replace('http://businessvoipcenter.com/', '', $_GET['destination']);
            $fixed_path = str_replace('___.html', '', $fixed_path);
            unset($_GET['destination']);
            watchdog('BVC Redirect from ___.html 404', $fixed_path . ' ------- >'. print_r($_GET, TRUE) . ' '. print_r($_SERVER, TRUE), NULL, WATCHDOG_WARNING);
            drupal_goto($fixed_path);
          }
          elseif (strpos($_GET['destination'], 'apple-touch-icon') !== FALSE) {
            watchdog('BVC What is apple-touch-icon???? 404', print_r($_GET, TRUE) . ' '. print_r($_SERVER, TRUE), NULL, WATCHDOG_WARNING);
          }
          ...
          echo bvc_blocks_getBlockThemed(array('module' => 'search', 'delta' => 'search-form', 'no_subject' => TRUE, 'class' => 'search-on-404', 'shadow' => FALSE)),
               '<br/><p>Or, try one of the links below.</p>';
          
          
          // Get content of the Sitemap.
          $sitemap_body = cache_get('bvc_sitemap_body');
          if (!$sitemap_body || empty($sitemap_body->data)) {
            ...
            cache_set('bvc_sitemap_body', $sitemap_body);
          }
          else 
          {
